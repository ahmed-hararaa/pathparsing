...
type TestPathProxy struct {
	called bool
}

func (p *TestPathProxy) MoveTo(x, y float64) {
	p.called = true
}

func (p *TestPathProxy) LineTo(x, y float64) {
	p.called = true
}

func (p *TestPathProxy) CubicTo(x1, y1, x2, y2, x3, y3 float64) {
	p.called = true
}

func (p *TestPathProxy) Close() {
	p.called = true
}

func assertValidPath(input string) {
	proxy := TestPathProxy{}
...
func assertInvalidPath(input string) {
	proxy := TestPathProxy{}
	err := WriteSvgPathDataToPath(input, &proxy)
	if err == nil {
		panic("No error")
	}
}
...
func TestMissingCommandsNumbersFlags(t *testing.T) {
